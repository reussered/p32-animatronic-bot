...
foreach ($jsonFile in $componentJsons) {
    try {
        ...
        if ($jsonFile.Directory.Name -eq "positioned") {
            if (-not $jsonContent.component_name) { $missingFields += "component_name" }
            if (-not $jsonContent.hardware_type) { $missingFields += "hardware_type" }
            if (-not $jsonContent.position) { $missingFields += "position" }
        }
        elseif ($jsonFile.Directory.Name -eq "hardware") {
            if (-not $jsonContent.hardware_type) { $missingFields += "hardware_type" }
            if (-not $jsonContent.component_name) { $missingFields += "component_name" }
        }
        elseif ($jsonFile.Directory.Name -eq "interfaces") {
            if (-not $jsonContent.interface_id) { $missingFields += "interface_id" }
            if (-not $jsonContent.interface_type) { $missingFields += "interface_type" }
        }
        
        # Check for generic incomplete indicators
        if ($jsonContent.component_name -eq "UNDEFINED" -or 
            $jsonContent.hardware_type -eq "UNDEFINED" -or
            $jsonContent.PSObject.Properties.Value -contains "TODO" -or
            $jsonContent.PSObject.Properties.Value -contains "PLACEHOLDER") {
            $isIncomplete = $true
        }
        
        if ($missingFields.Count -gt 0 -or $isIncomplete) {
            $incompleteJsons += [PSCustomObject]@{
...
foreach ($sourceFile in $sourceFiles) {
    try {
        ...
        $warningCount++
        if ($Verbose) { Write-Warning "Could not scan: $($sourceFile.FullName)" }
    }
}
...
foreach ($headerFile in $headerFiles) {
    try {
        ...
        $warningCount++
        if ($Verbose) { Write-Warning "Could not scan: $($headerFile.FullName)" }
    }
}
...
foreach ($cmake in $cmakeFiles) {
    try {
        ...
        $warningCount++
        if ($Verbose) { Write-Warning "Could not scan CMake: $($cmake.FullName)" }
    }
}
