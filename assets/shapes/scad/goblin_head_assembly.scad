...
function calculate_static_torque(weight_grams, moment_arm_mm) =
    ...
    force_n * moment_arm_m;                   // Torque in Nm
...
function calculate_dynamic_torque(weight_grams, moment_arm_mm, angular_accel_rad_s2) =
    ...
    moment_of_inertia * angular_accel_rad_s2; // Torque in Nm
...
// Convert Nm to ozin for motor specifications
...
echo(str("Required Motor Torque: ", round(required_torque_nm*1000)/1000, " Nm (", round(required_torque_ozin*10)/10, " ozin)"));
echo(str("Recommended Motor: NEMA 17 with ", ceil(required_torque_ozin/10)*10, " ozin holding torque"));
echo("============================================");
