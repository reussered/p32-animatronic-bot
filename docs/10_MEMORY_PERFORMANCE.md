...
## 5. Common Performance Mistakes

### A. String Operations in act()

**DON'T**: Build strings every loop
...
{
    char buffer[100];
    sprintf(buffer, "Temp: %d C, Battery: %d%%", temp, battery);
    draw_text(buffer);  // Allocates memory internally
}
```

**DO**: Update only on change

```cpp
static char status_text[100] = {0};
static int last_temp = -999;
static int last_battery = -999;

void display_act(void)
{
    if (temp != last_temp || battery != last_battery)
    {
        sprintf(status_text, "Temp: %d C, Battery: %d%%", temp, battery);
        last_temp = temp;
        last_battery = battery;
    }
    
    draw_text(status_text);  // No allocation
}
```
