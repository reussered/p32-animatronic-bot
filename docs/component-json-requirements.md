...
### **Required Fields (All JSON Files)**
...

{
  ...
  # Note: "relative_filename" is optional and is not required; it may appear for documentation or tooling convenience
...
esp_err_t parse_component_config(const char* json_string) {
...

    {
      ...
      // "relative_filename": "config/components/positioned/component.json",  # optional
  ...
  - Holds the actual `init()`/`act()` definitions used by the subsystem.
    ...
    - For the first encounter of a component:
      ...
      - Emits nested structs mirroring the components JSON keywords (excluding `components`).
    ...
    Encountering a `controller` keyword marks a subsystem boundary and triggers the creation of six files named after the components `name` field:
    ...
    `{name}_main.cpp` includes the dispatch tables and component-functions headers, providing the subsystems local main loop. All generated files are appended to `CMakeLists.txt` so PlatformIO/ESP-IDF builds pick them up automatically.
    ...
    When a subsystem declares a `controller`, the generator loads that boards pin capabilities and constructs assignable arrays:
    ...
    `spi_bus_init()` (and analogous helpers) pull from these arrays and record the allocation. They are invoked exactly once per hardware interface instancee.g., two `goblin_eye` displays mean two SPI allocations.
