...
├─ mouth                 ├─ esp_now_mesh          ├─ elbow
└─ ears                  └─ personality           └─ gripper
```

**Inter-Subsystem Communication**: Global Shared Memory (GSM) + ESP-NOW
...
| Different subsystems (different chips) | GSM + ESP-NOW | `GSM.read<Mood>()` / `GSM.write<Mood>()` |
