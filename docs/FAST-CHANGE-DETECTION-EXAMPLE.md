...
└── components/
    ...
    └── esp_now_mesh.cpp  ← Broadcasts changes
```
...
// POD struct for ESP-NOW transmission
typedef struct {
    // Metadata (not compared for changes)
    uint32_t version;
    uint32_t timestamp_ms;
    uint8_t source_node_id;
    
    // Data fields (these ARE compared)
    int8_t mood_serialized[9];        // Mood class → array
    uint8_t personality_serialized[32]; // Personality class → array
