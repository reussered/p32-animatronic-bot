...
| **Props** | 8 | APC 10×5 Carbon Fiber | $10 | 2 spare sets |
...
# Step 2: Increase throttle from 1000 µs to 2000 µs in steps
...
foreach ($test in $throttle_tests) {
    Write-Host "Testing PWM $($test.pwm_us) µs ($($test.label) throttle)..."
...
{
    // Convert µs to duty cycle (0–4095 for 13-bit resolution)
    // PWM period = 1/490 Hz ≈ 2041 µs
...
void flight_control_task(void *arg)
{
    ...
    while (1) {
        ...
        if (system_armed) {
            // Test: Ramp throttle from 1000 to 1500 µs over 5 seconds
...
## PART 6: TESTING SEQUENCE (Week 1–3)

### Day 1–2: Structural Assembly
...
### Day 3–5: Motor Testing

```bash
# Objective: Validate all 4 motors spin independently
# Manually send PWM commands via serial terminal:
# "motor1 1100" → Motor 1 at 1100 µs (gentle spin)
# "motor1 1500" → Motor 1 at 1500 µs (50% throttle)
# "motor1 2000" → Motor 1 at 2000 µs (100% throttle)
