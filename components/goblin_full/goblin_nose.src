...
static const char *goblin_nose_TAG = "GOBLIN_NOSE";
...
esp_err_t goblin_nose_init(void) {
    ...
    ESP_LOGI(goblin_nose_TAG, "Component initialized");
    return ESP_OK;
}
...
void goblin_nose_act(void) {
    ...
    if (g_loopCount % 1000 == 0) {
        printf("ACT: goblin_nose - distance %.1fcm (loop %llu)\n", simulated_distance, g_loopCount);
    }
    return;
#endif
    ...
    ESP_LOGD(goblin_nose_TAG, "Proximity sensor reading: %.1f cm - loop %llu", simulated_distance, g_loopCount);
}
