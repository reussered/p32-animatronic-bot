...
// These functions would be used by other components to get display properties.
...
void generic_spi_display_act(void) {
    ...
    uint8_t y = 0, h = 240;
    uint8_t row_data[] = { (uint8_t)(y >> 8), (uint8_t)(y & 0xFF), (uint8_t)((y + h - 1) >> 8), (uint8_t)((y + h - 1) & 0xFF)};
    generic_spi_write_data(cur_spi_pin.handle, row_data, 4);

    // Memory write
    spi_write_command(cur_spi_pin.handle, GC9A01_RAMWR);

    // For testing, fill with a solid color that depends on the CS pin
    uint16_t color = (cur_spi_pin.cs % 2 == 0) ? 0xF800 : 0x07E0; // Red or Green
    size_t buffer_size = 240 * 2;
    uint8_t* pixel_buffer = (uint8_t*)malloc(buffer_size);
    if (!pixel_buffer) {
        ESP_LOGE(TAG_generic_spi_display, "Failed to allocate pixel buffer");
        return;
    }
    for (size_t i = 0; i < 240; i++) {
        pixel_buffer[i * 2] = color >> 8;
        pixel_buffer[i * 2 + 1] = color & 0xFF;
    }

    for (uint32_t i = 0; i < 240; i++) {
        generic_spi_write_data(cur_spi_pin.handle, pixel_buffer, buffer_size);
    }
    free(pixel_buffer);
    
    // Add a delay to make the color cycling visible
    vTaskDelay(pdMS_TO_TICKS(500));
}
