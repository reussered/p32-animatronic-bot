...
#ifdef SIMPLE_TEST
    printf("ACT[%lu]: heartbeat - hitCount:1\n", loopCount);
    return;
#endif
    ESP_LOGI(TAG_HEARTBEAT, "Loop %lu: System heartbeat", loopCount);
}
...
void p32_comp_network_monitor_act(uint32_t loopCount) {
    ...
    if (loopCount % 1 == 0) printf("ACT[%lu]: network_monitor - hitCount:1\n", loopCount);
    return;
#endif
    ESP_LOGI(TAG_NETWORK_MONITOR, "Loop %lu: Network monitoring and loop timing", loopCount);
