...
esp_err_t goblin_nose_init(void)
{
    ...
    // Configure GPIO4 as output (trigger)
    ...
    // Configure GPIO5 as input (echo with voltage divider)
    ...
    ESP_LOGI(TAG, "HC-SR04 sensor initialized (GPIO4=trigger, GPIO5=echo)");
    return ESP_OK;
}
...
void goblin_nose_act(void)
{
    ...
    while (gpio_get_level((gpio_num_t)5) == 0)  // GPIO5 = ECHO pin
    {
    ...
    while (gpio_get_level((gpio_num_t)5) == 1)  // GPIO5 = ECHO pin
    {
        if ((esp_timer_get_time() - start_time) > timeout_us)
        {
