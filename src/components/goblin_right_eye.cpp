...
esp_err_t goblin_right_eye_init(void) {
    ...
    right_eye_animation_buffer = getBuffer();
    ...
    ESP_LOGI(TAG, "Right eye initialized with %u animation frames on %ux%u display", 
             right_eye_frame_count, frame_width, frame_height);
    return ESP_OK;
}
...
void load_current_frame_to_buffer(void) {
    ...
    // TODO: Re-enable SharedMemory access after fixing compilation issues
    ...
    uint8_t viewer_distance = 128; // Default distance for now (env_ptr != nullptr) ? env_ptr->distance_cm : 128; // Default if not available
    ...
    switch (right_eye_current_frame) {
        ...
        case 1: // Slight squint with focus adaptation
            for (uint32_t i = 0; i < total_pixels; i++) {
                uint32_t y = i / frame_width;
                uint32_t squint_top = frame_height * 50 / 240;    // Scale squint boundaries
                uint32_t squint_bottom = frame_height * 190 / 240;
        ...
        case 3: // Narrowed focus with distance adaptation
            for (uint32_t i = 0; i < total_pixels; i++) {
                uint32_t y = i / frame_width;
                if (y > frame_height * 70 / 240 && y < frame_height * 170 / 240) {
                    uint32_t x = i % frame_width;
                    uint32_t distance = (x - center_x) * (x - center_x) + (y - center_y) * (y - center_y);
        ...
        case 5: // Satisfied look with focus adaptation
            for (uint32_t i = 0; i < total_pixels; i++) {
                uint32_t y = i / frame_width;
                if (y > frame_height * 40 / 240 && y < frame_height * 200 / 240) {
                    uint32_t x = i % frame_width;
                    uint32_t distance = (x - center_x) * (x - center_x) + (y - center_y) * (y - center_y);
