...
static const char *TAG = "TEST_HEAD";

// Component initialization function - NO ARGUMENTS
esp_err_t test_head_init(void)
{
    ESP_LOGI(TAG, "Initializing test head subsystem for dual GC9A01 displays");
    ...
    ESP_LOGI(TAG, "Test head subsystem initialized - ready for display validation");
    return ESP_OK;
}
...
void test_head_act(void)
{
    ...
    {
        ESP_LOGD(TAG, "Test head coordination active (loop %llu)", g_loopCount);
