...
static const char *NETWORK_MONITOR_TAG = "NETWORK_MONITOR";
...
esp_err_t network_monitor_init(void) {
    ...
    ESP_LOGI(NETWORK_MONITOR_TAG, "Network monitor initialized");
    return ESP_OK;
}
...
void network_monitor_act(void) {
    ...
    if (g_loopCount % 10000 == 0) {
        ESP_LOGD(NETWORK_MONITOR_TAG, "Network status check - loop %llu", g_loopCount);
    }
}