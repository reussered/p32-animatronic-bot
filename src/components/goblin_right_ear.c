// P32 Component: goblin_right_ear
...
static const char *TAG = "GOBLIN_RIGHT_EAR";

// Component: Right ear HW-496 microphone for directional audio input
esp_err_t goblin_right_ear_init(void) {
#ifdef SIMPLE_TEST
    printf("INIT: goblin_right_ear - Right ear HW-496 microphone for directional audio input\n");
    return ESP_OK;
#endif
...
void goblin_right_ear_act(uint64_t loopCount) {
#ifdef SIMPLE_TEST
    if (loopCount % 1000 == 0) {
        printf("ACT: goblin_right_ear - active (loop %llu)\n", loopCount);
    }
    return;
#endif
