...
Write-Host "`n⚙️  Setting permanent user environment variables..." -ForegroundColor Yellow
$EnvVars = @{
...
if (Test-Path $OldPlatformIODir) {
    ...
    try {
        ...
        Write-Host "   ⚠️  Migration failed: $($_.Exception.Message)" -ForegroundColor Red
...
Write-Host "🏗️  Next: Run 'pio run -e test_bot_minimal' to test the new configuration" -ForegroundColor Yellow