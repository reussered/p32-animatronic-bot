...
void compute_motor_commands(
    ...
    float roll_cmd,           // [−1.0–1.0] (δ left, + right)
    float pitch_cmd,          // [−1.0–1.0] (− forward, + backward)
    float yaw_rate_cmd,       // [−1.0–1.0] (− CCW, + CW)
    float motor_out[4])       // Output PWM [0.0–1.0]
{
    ...
    // Clamp to [0.0–1.0] to prevent saturation
...
1. **ESC Initialization** (per ESC)
   ...
   - Gradually increase throttle from 0–100%
...
2. **Static Thrust Measurement**
   ...
   - Set throttle to 0% (idle ~1000 µs PWM)
   - Record baseline weight (should be motor weight only)
   - Increase to 50% throttle (1500 µs PWM), wait 5 sec, record scale reading
...
4. **ESC PWM Response**
   ...
   - Send 1000 µs pulse (idle), verify no spin
   - Increase to 1100 µs, verify gentle spin
   - Increase to 1500 µs, record response time (<100 ms expected)
...
def validate_power_budget():
    ...
    print(f"  Target: 10–12 min mixed flight")
