        ...
        try {
            ...
            foreach ($instance in $found) {
                ...
                $INSTANCES += @{
                    ...
                    Size = if (Test-Path "$fullPath\src") { 
                        (Get-ChildItem -Path "$fullPath\src" -Recurse -File -ErrorAction SilentlyContinue | Measure-Object -Property Length -Sum).Sum 
                    } else { 0 }
                    FileCount = if (Test-Path "$fullPath\src") { 
                        (Get-ChildItem -Path "$fullPath\src" -Recurse -File -ErrorAction SilentlyContinue | Measure-Object).Count 
                    } else { 0 }
                }
