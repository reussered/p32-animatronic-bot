# Goblin Camera System Research

## Autofocus + Gimbaled Positioning Investigation

**Date:** November 6, 2025

**Objective:** Investigate camera + mechanical control options for animatronic eyes with autofocus and 3-axis positioning (pan, tilt, roll).

---

## 1. Camera Options Analysis

### 1.1 Raspberry Pi Camera Module v3 (IMX708)
...
### 1.2 ArduCAM OV5647 / IMX219 / IMX477 (Various Models)
...
**Limitation:** None of these models have electronic autofocus.
...
- Native autofocus (no moving parts in lens barrel; AF via register writes)
...
- Low resolution
- Mechanical focus unreliable for servo-based positioning

### 1.6 USB UVC Generic Webcams
...
### 2.1 Commercial Pan-Tilt-Roll Gimbal Mounts

Example: Feetech SGS16/FT-TBS15 Servo Gimbal

- Axes: Pan (+/-150 degrees), Tilt (+/-90 degrees), Roll (+/-45 degrees)
...
### Option A: Two-Axis Servo Gimbal + Autofocus Camera
...
For the goblin animatronic camera system:

- **Best balance:** ArduCAM IMX585 + 2-axis servo gimbal
- **Key advantage:** Electronic autofocus (reliable, no mechanical wear)
- **Cost:** $70-160 per system ($140-320 for stereo pair)
- **Integration:** I2C autofocus control + PWM servo control on standard ESP32
- **Fallback:** Graceful degradation to fixed focus if I2C autofocus fails

See CAMERA_COST_RELIABILITY_ANALYSIS.md for detailed cost breakdown and reliability comparison.
