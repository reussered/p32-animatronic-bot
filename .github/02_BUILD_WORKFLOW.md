...
python tools\generate_tables.py <subsystem_name>
...
⚠️ **CRITICAL WORKSPACE RULE**: All build operations must reference files from the local workspace directory, never from repository cache or external sources. Paths may be relative to the workspace root (e.g., `tools/generate_tables.py`) or absolute, but must resolve to workspace locations on the hard drive. Never use repository URLs or remote paths.
