...
###  MANDATORY VERIFICATION GATE - READ BOTH RULE FILES BEFORE PROCEEDING
...
4.  Are you creating ANY new files (code, config, scripts)?
   ...
   - Question 1: Will this file be included in the build system, project repository, or referenced by other files?
     - If YES: **MUST be 100% ASCII only** (0x00-0x7F). No Unicode allowed. EVER.
     - If NO (standalone documentation ONLY): Check if it meets EXCEPTION criteria (Rule 3b)
       - Must state "STANDALONE DOCUMENTATION - NOT PART OF PROJECT BUILD" at top
       - Must be outside src/, include/, config/, tools/
       - Must never be imported or referenced by code
...
## RULE 1B: NO COMMITS WITHOUT EXPLICIT APPROVAL

The agent MUST NEVER commit or push code without explicit user approval.
...
generic_spi_display.json:
  ...
  components: []  (leaf component - empty array indicates no children)

```



**Note:** Components CAN have multiple children in `components: []` array, but in this pipeline each has only one. Leaf components MUST have an empty `components: []` array.
...
**This is the CORE of how the build system works. Understand this completely.**
...
1. DEPTH-FIRST TRAVERSAL: Walk the tree in order, going deep before going wide
...
2. FOR EACH COMPONENT ENCOUNTERED:

   a. Load component.json file
   
   b. AGGREGATION CHECK - Is this the FIRST TIME we've seen this component name?
      - FIRST ENCOUNTER: Aggregate .src file into {component_name}_component_functions.cpp
      ...
      - DUPLICATE ENCOUNTER: Skip aggregation (already aggregated the first time)
      
   c. DISPATCH TABLE ENTRY: Add component to dispatch table (ALWAYS, even if duplicate)
...
Result: Dispatch tables execute in pipeline order, creating data flow, with duplicates expected and correct.



### RULE 4B: FILE ORGANIZATION AND SUBSYSTEM STRUCTURE



**CRITICAL: Consolidation point for understanding the entire parsing and generation process.**

This section explains where and how files are organized during generation. RULE 4A (above) explains the parsing algorithm. Together they define the complete generation process.
...
**Example for `goblin_head` subsystem - 6 files generated:**

1. **`src/subsystems/goblin_head/goblin_head_component_functions.cpp`** - Aggregated component code (from .src files)

2. **`include/subsystems/goblin_head/goblin_head_component_functions.hpp`** - Aggregated component headers (from .hdr files)

3. **`src/subsystems/goblin_head/goblin_head_dispatch_tables.cpp`** - Dispatch table implementation with ALL component entries

4. **`include/subsystems/goblin_head/goblin_head_dispatch_tables.hpp`** - Dispatch table declarations

5. **`src/subsystems/goblin_head/goblin_head_main.cpp`** - Subsystem main entry point and FreeRTOS task setup

6. **`include/subsystems/goblin_head/goblin_head_main.hpp`** - Main entry point declarations



**Rationale for Bot/Subsystem-Specific File Names:**

- Each subsystem (head, torso, left_arm, etc.) generates independently
...
- Preserves previously-generated working configurations while new subsystems are added



**WHERE DUPLICATES ARE ALLOWED vs. WHERE THEY ARE AVOIDED:**

| Aspect | Location | Duplicates? | Reason |
|--------|----------|------------|--------|
| **Aggregated .cpp code** | `{name}_component_functions.cpp` | **AVOIDED** | Code aggregated ONLY on first encounter - reused for all subsequent references |
| **Aggregated .hpp code** | `{name}_component_functions.hpp` | **AVOIDED** | Headers aggregated ONLY on first encounter - reused for all subsequent references |
| **Dispatch table entries** | `{name}_dispatch_tables.cpp` | **ALLOWED** | Dispatch table contains EVERY component encountered - duplicates expected and correct |
| **Component pipeline** | JSON `components: []` array | **ALLOWED** | Same component can appear multiple times in different pipelines - each gets execution entry |
| **Runtime execution** | Main loop calling dispatch table | **ALLOWED** | Same component code executes multiple times per loop - different data contexts managed ONLY via file-scoped static variables (NOT function arguments) |

**Key Principle**: Code (`.cpp`/`.hpp`) is never duplicated. Execution (dispatch table entries) absolutely can and should have duplicates when the same component appears in multiple pipelines.



**CRITICAL - Component Function Signatures Are FIXED:**

Component functions **NEVER take arguments**. Ever. The signature is always:

```cpp
esp_err_t {component_name}_init(void);  // NO ARGUMENTS
...
- Shared State Classes: `shared/{ClassName}.hpp` (Mood, Environment, Personality, SysTest, etc.)

- Shared Header Definitions: `config/shared_headers/{name}.hpp` (color_schema, etc.)
...
- Component aggregators: `src/components/{component_name}.cpp`
...
- Subsystem main: `src/subsystems/{subsystem_name}/{subsystem_name}_main.cpp`

- Subsystem headers: `include/subsystems/{subsystem_name}/*.hpp`



**OBSOLETE (No longer used):**

- `config/components/creature_specific/` - Functionality moved to `config/bots/bot_families/`

- Standalone `config/hardware/`, `config/driver/`, `config/interfaces/` - MOVED to `config/components/`

- Standalone `config/behaviors/`, `config/moods/` - MOVED to `config/components/behaviors/`
...
**Generation Process** - See RULE 4 (Parser Job) for complete parsing details:

- Generate bot/subsystem-specific filenames from bot definition JSON using pattern `{bot_family}_{subsystem_name}`

- RULE 4 handles: depth-first traversal, aggregation on first encounter, dispatch table duplicate entries

- **WHERE DUPLICATES ARE ALLOWED** (and expected): In dispatch tables when same component appears in multiple pipelines

- **WHERE DUPLICATES ARE AVOIDED**: In aggregated .cpp/.hpp files (code aggregated only once, reused for all references)
...
** CRITICAL**: Any changes to the classes in this directory invalidates all of the generated files in `src/` and `include/`. Regenerate immediately after modifying any `/shared` class.
...
## RULE 25: ASCII ENCODING ONLY - MANDATORY FOR ALL PRODUCTION FILES



**ABSOLUTE REQUIREMENT**: ALL files in production must be pure ASCII (0x00-0x7F). NO UTF-8 BOM. NO Unicode characters.



**APPLIES TO (NO EXCEPTIONS - PRODUCTION FILES ONLY):**

- All C++ source files (.cpp, .hpp, .h, .src, .hdr)
- All JSON configuration files (.json)
- All Python scripts (.py)
- All PowerShell scripts (.ps1)
- All generated files (dispatch tables, component aggregators, etc.)
- All documentation in `src/`, `include/`, `config/`, `tools/` directories
- All files referenced in build system or imported by other code
- All files that are part of the version control for the project



**EXPLICIT RULE - NO UNICODE IN PRODUCTION:**
...
If creating a file (`.md`, `.txt`) that is:

1. **Standalone** - Not included in any build process
2. **Unincluded** - Never imported, included, or referenced by any code file
3. **Outside production** - Lives in root directory or clearly separated documentation folder (NOT in src/, include/, config/, tools/)
4. **Explicitly labeled** - MUST contain at top of file:
   ```
   [STANDALONE DOCUMENTATION - NOT PART OF PROJECT BUILD - HUMAN REFERENCE ONLY]
   ```
5. **Not version-controlled** - Not intended for production Git branches (or clearly marked as non-binding)

Then: Unicode characters are permitted ONLY for improved human readability of research/analysis/decision documents.

Examples that qualify:
- `CAMERA_RESEARCH_FINDINGS.md` (research only, human reference)
- `FLYING_DRAGON_DECISION_NOTES.md` (analysis document, not used in build)
- `ARCHITECTURE_NOTES.md` (historical design document)

Examples that DO NOT qualify:
- `README.md` in root (part of project identity)
- Any file in `src/`, `include/`, `config/`, `tools/`
- Any file imported by C++, Python, or PowerShell
- Any file used in documentation generation for the build
...
1. Assume ASCII-only is required
2. If Unicode seems necessary, ask: "Is this file ever included in the build or referenced by code?"
3. If answer is YES: Rewrite in ASCII. No exceptions.
4. If answer is NO: Check exception criteria above. If all 5 criteria are met: Unicode allowed.
5. If unsure: Always default to ASCII.

**Cleanup scripts to remove Unicode are a FAILURE, not a solution.** Files should be planned for ASCII from the first character. Do not write Unicode then cleanup afterward.
