...
5. **Component Type**: `"type": "DISPLAY_DRIVER"` - Categorizes component (DISPLAY_DRIVER, MICRO_CONTROLLER_MODULE, etc.)
...
esp_err_t parse_component_config(const char* json_string) {
  ...
  - Holds the actual `init()`/`act()` definitions used by the subsystem.
    ...
    - [ ] Generated dispatch tables are expected to contain duplicates.
