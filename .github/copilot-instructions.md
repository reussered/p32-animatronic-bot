...
- **Never compile .src or .hpp files directly**: These are aggregated into `p32_component_functions.cpp` by `generate_tables.py`. The original `.src` and `.hdr` files live in `config/` locations (these are permanent source files); files in `src/` and `include/` are generated/temporary and should not be edited manually.
...
**Dispatch Tables**: Auto-generated from JSON. The `initTable.h` and `actTable.h` files contain function pointers for every component instance found during the recursive JSON tree walk. **Duplicates are intentionally included** for every time a component is referenced.

**Component Aggregation**: The `p32_component_functions.cpp` and `p32_component_functions.hpp` files are built by aggregating the source code from the permanent `.src` and `.hdr` files found in `config/components/`. They are not individual files per component.
...
- **Source Components**: `config/components/{name}.src` (permanent source code)
- **Header Components**: `config/components/{name}.hdr` (permanent headers)
- **Aggregated Source**: `src/p32_component_functions.cpp` (generated by concatenating all required `.src` files)
- **Aggregated Headers**: `include/p32_component_functions.hpp` (generated by concatenating all required `.hdr` files)
- **Dispatch Tables**: `src/subsystems/{subsystem}/*_dispatch_tables.cpp` (generated)
