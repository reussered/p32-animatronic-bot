...
| Manually declared `use_fields` variable | `use_fields` are auto-generated in aggregated .cpp; manual declarations cause conflicts |
...
**DON'T**: Manually declare `use_fields` variables - they are auto-generated

```cpp
// WRONG: Don't declare use_fields variables manually
// int display_width = 240;  // Causes multiple definition with auto-generated
```

Instead, the generator auto-inserts assignments in `init()` and `act()` functions:

```cpp
// In your .src file - you write this:
void goblin_eye_act(void)
{
    // Generator inserts these assignments at function start:
    // int display_width = 240;     // auto-inserted from use_fields
    // int display_height = 240;    // auto-inserted from use_fields
    
    int display_size = display_width * display_height;
    if (display_size > 0)
    {
        render_eye(display_width, display_height);
    }
}
```
