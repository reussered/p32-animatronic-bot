...
void app_main(void)
{
    ...
    {
        execute_act_table();
        vTaskDelay(1);  // 1ms delay - minimum for watchdog compliance (~1000Hz loop)
    }
}
```
...
|------------|-----------|----------|
| 1 | Every loop (~1000Hz) | Critical sensors, fast rendering |
| 2 | Every 2nd loop (~500Hz) | Servo updates, LED patterns |
| 5 | Every 5th loop (~200Hz) | Behavior logic, mood updates |
| 10 | Every 10th loop (~100Hz) | Slow sensors, diagnostics |
| 100 | Every 100th loop (~10Hz) | Status reporting, cleanup |
...
[
  {
    ...
    "timing": { "hitCount": 1 }  // 1000Hz - smooth animation
  },
  {
    "component": "goblin_servo",
    "timing": { "hitCount": 2 }  // 500Hz - servo control
  },
  {
    "component": "goblin_behavior",
    "timing": { "hitCount": 10 }  // 100Hz - decision making
  }
]
```
