...
- Component names MUST be globally unique - no duplicates allowed --> specified by the `name` field in the components JSON file
...
if the component in the components:[] array was specified with a template like goblin_eye<GC9A01>  the act() and init() functions become templated functions and the prototype generated byb the parser is
temnplate< class T >
esp_err_t component_name_init<T>()
and 
temnplate< class T >
void component_name_act<T>()

the functions in the dispatch table look like
component_name_init<GC9A01>()  // the specifc type invoked in the component
the functions in the source are
template<class T >
void component_name<T>()
etc.
...
- **Creatures/Bots**: each specific creature is identified by a jason file in `config/bots/bot_families/{family}/{name}.json`.  
	- inside each family folder there are folders corresponding to the various subsytems of the {family}  for example in the goblins folder are the folders corresponding to the various subsystems.  for goblins it is head, torso, arms, legs, feet, hands.
	for dragons there should be the folders head, body_segment, wings, legs, feet
	- inside each of these folders (head for example) go all of the creature specific components that belong to that subsyetm, so under the config/bots/bot_families/goblins/head folder would go goblin_head as well as goblin_left_eye, goblin_eye, goblin_nose, etc
	in the examples where there are folders inside the family (for example cats), the subfolders like wildcats, domestic, etc. are removed and the creaturee deinitions inside each are moved into the higher level `cats` folder.  if there are duplicates, the names are changed to reflect the subfamility type.  this is because all cats have the same body parts.
...
- **Generated Software**: Created by python scripts in `/tools` go into regular `src/` and `include/` folders. This includes dispatch tables and component functions `{subsystem}_dispatch_tables.hpp/.cpp` and `{subsystem}_component_functions.hpp/.cpp`. Only the last copy generated is kept. 
